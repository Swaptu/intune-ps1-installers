...
# Intune-deployed script — silently upgrades Adobe Acrobat Reader
# Logs to: C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\
# ============================================================

$LogFile = "C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\Update-AdobeReader.log"
$LogDir  = Split-Path $LogFile

if (-not (Test-Path $LogDir)) { New-Item -Path $LogDir -ItemType Directory -Force | Out-Null }

$Apps = @(
    @{ Name = "Adobe Reader"; IdPrefix = "Adobe.Acrobat.Reader"; Processes = @("AcroRd32", "Acrobat") }
)
...
try {
    Invoke-LogRotation
    Write-Log "====== Update-AdobeReader started ======"
    ...
    Write-Log "====== Update-AdobeReader finished ======"
    exit ($anyFailed ? 1 : 0)
}
catch {
    Write-Log "====== Update-AdobeReader FAILED: $_ ======" -Level ERROR
    exit 1
}
