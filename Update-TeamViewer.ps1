...
# Intune-deployed script — silently upgrades TeamViewer
# Logs to: C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\
# ============================================================

$LogFile = "C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\Update-TeamViewer.log"
$LogDir  = Split-Path $LogFile

if (-not (Test-Path $LogDir)) { New-Item -Path $LogDir -ItemType Directory -Force | Out-Null }

$Apps = @(
    @{ Name = "TeamViewer"; IdPrefix = "TeamViewer.TeamViewer"; Processes = @("TeamViewer") }
)
...
try {
    Invoke-LogRotation
    Write-Log "====== Update-TeamViewer started ======"
    ...
    Write-Log "====== Update-TeamViewer finished ======"
    exit ($anyFailed ? 1 : 0)
}
catch {
    Write-Log "====== Update-TeamViewer FAILED: $_ ======" -Level ERROR
    exit 1
}